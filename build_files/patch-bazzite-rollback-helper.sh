...
awk '
  /^signing_scheme\(\) \{/ {
    ...
    print "  local current=$(rpm-ostree status -b --json | jq -r '\''.deployments[] | select(.booted == true) | .["container-image-reference"]'\'')"
    ...
    print "  local current"
    print "  current=$(rpm-ostree status -b --json | jq -r '\''.deployments[] | select(.booted == true) | .["container-image-reference"]'\'')"
